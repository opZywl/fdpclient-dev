...
public class NlSetting{
    ...
    public void draw(int mx, int my){
        ...
        Fonts.Nl_18.drawCenteredString("fdpclient @ 2019", x+ ((float) 160 /2) , y + 65 + (Fonts.Nl_18.getHeight() + 5) * 4 + 7, Light ? new Color(95,95,95).getRGB() :-1);
    ...
    public static void NLOutline(String str, FontRenderer fontRenderer, float x, float y, int color, int color2, int w, float size) {
        fontRenderer.drawCenteredString(str, x + ((float) w /2) + size, y, color2, false);
        fontRenderer.drawCenteredString(str, x+ ((float) w /2) , y - size, color2, false);
        fontRenderer.drawCenteredString(str, x+ ((float) w /2) , y, color, false);
    }
