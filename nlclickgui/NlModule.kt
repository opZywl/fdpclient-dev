...
class NlModule(var NlSub: NlSub, var module: Module, var lef: Boolean) {
    ...
    private fun shouldDisplaySetting(setting: Value<*>) = !setting.name.equals("Hide", ignoreCase = true)
    ...
    init {
        ...
        for (setting in module.values) {
            if (!shouldDisplaySetting(setting)) continue
    ...
    fun calcHeight(): Int {
        var h = 20
        for (s in module.values.stream().filter { obj: Value<*>? -> obj!!.shouldRender() && shouldDisplaySetting(obj) }
    ...
    fun draw(mx: Int, my: Int) {
        ...
        if (module.values.isEmpty()) {
            Fonts.Nl.Nl_22.Nl_22!!.drawString(
                "No settings.",
                x + 100 + posx,
                y + posy + scrollY + NeverloseGui.HEADER_HEIGHT + 32,
                if (getInstance().light) Color(95, 95, 95).rgb else -1
            )
        }
    }
