...
class NlTab(val type: Category, val y2: Int) {
    ...
    init {
        ...
        for (subCategory in type.subCategories) {
            nlSubList.add(NlSub(subCategory, y2 + y3))
            y3 += 18
        }
    }

    fun draw(mx: Int, my: Int) {
        Fonts.Nl_16.drawString(type.name, (x + 10).toFloat(), (y + y2).toFloat(), if (NeverloseGui.getInstance().light) Color(194, 196, 198).rgb else Color(66, 64, 62).rgb)
                ...
                for (nlModule in nlSub.nlModules) {
                    for (nlSetting in nlModule.downwards) {
                        if (nlSetting is Numbersetting) {
                            nlSetting.percent = 0.0
                        }
                        if (nlSetting is BoolSetting) {
                            if (nlSetting.toggleAnimation.direction == Direction.FORWARDS) nlSetting.toggleAnimation.reset()
                        }
                    }
                    if (nlModule.toggleAnimation.direction == Direction.FORWARDS) nlModule.toggleAnimation.reset()
                }
            }
